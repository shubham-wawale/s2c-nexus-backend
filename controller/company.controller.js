...
companyController.post('/login', (req, res) => {
  ...
  CompanyInfo.findOne({ email }).then((company) => {
    if (!company) {
      res.json({ success: false, message: `Company with email ${email} does not exist.` });
    } else if (company.hashedPassword != sha256(password)) {
...
companyController.post('/signup', (req, res) => {
  if (req.body) {
    const {
      email, password, name, type, yearOfEstablishment, vision,
    } = req.body;
    const companyData = {
      email,
...
companyController.get('/driveInfo', (req, res) => {
  ...
  CompanyDrive.find({ _id: driveId }).then((drive) => {
    if (!drive) {
      ...
      res.json({ success: true, message: 'Drive retrieved', drive });
    }
  });
});
...
companyController.get('/drives', (req, res) => {
  ...
  CompanyDrive.find({ companyId }).then((drives) => {
    if (!drives) {
...
companyController.get('/appliedStudentsDrive', (req, res) => {
  const { driveId } = req.query;
  CompanyDrive.find({ driveId }).then((drive) => {
    if (!drive) {
      res.json({ success: false, message: 'No drives found.' });
    } else {
...
companyController.get('/getAllDetailsCount', (req, res) => {
  CompanyInfo.countDocuments({}).then((data) => {
    ...
    res.status(400).json({ success: false, error, message: 'Company Records could not be fetched.' });
  });
});
